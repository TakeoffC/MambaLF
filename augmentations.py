...
def resize_sample(sample, image_shape, image_interpolation=Image.Resampling.LANCZOS):
    """
...
def add_noise(img, mode="gaussian", percent=0.02):
    ...
    if mode == "gaussian":
        ...
        if img.dtype not in [np.float32, np.float64]:
            ...
            img = np.clip(img.astype(float) + gauss, 0, np.iinfo(img.dtype).max)
        else:
            img = np.clip(img.astype(float) + gauss, 0, 1)
