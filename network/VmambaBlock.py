...
try:
    from .csms6s import flops_selective_scan_f as selective_scan_fn_v1
    from .csms6s import flops_selective_scan_ref as selective_scan_ref_v1
...
# 根据 self.dim 的值, 可以在特定维度上应用 Softmax
...
class VmambaBlock_down(nn.Module):
    def __init__(
            self,
            depths=[2],
            dims_input=32,
        ...
        for i_layer in range(self.num_layers):
            ...
            # _make_layer根据drop_path的长度确定有多少层
    ...
    # 在这里可以调整输入和输出的维度
    # 下采样，使特征图的尺寸减半
    def _make_downsample(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=True):
        # if channel first, then Norm and Output are both channel_first
    ...
    @staticmethod
    # 下采样时的卷积核更大
    def _make_downsample_v2(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        return nn.Sequential(
            (nn.Identity() if channel_first else Permute(0, 3, 1, 2)),
