...
def keep_shared_points(keypoints, descriptors, H, shape, keep_k_points=1000):
    ...
    selected_keypoints, selected_descriptors = select_k_best(keypoints, descriptors, keep_k_points)
...
def compute_homography(data, keep_k_points=1000):
    ...
    if m_keypoints.shape[0] > 4:

        H , _ = pydegensac.findHomography(matches[:, :2], matches[:, 2:4], ransac_thres=2)
    else:
        print(f"Not enough points to compute homography: {m_keypoints.shape[0]} points found")
        H = None
