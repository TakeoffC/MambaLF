...
def build_descriptor_loss(source_des, target_des, tar_points_un, top_kk=None, relax_field=4, eval_only=False):
    ...
    for b_id in range(batch_size):
        ...
        dmat = torch.sqrt(2 - 2 * torch.clamp(dmat, min=-1, max=1))  # [-1, 1]-->[-2, 2]-->[0, 4]-->[0, 2]
        ...
        for pyramid in range(len(relax_field_size)):
            ...
            loss += float(1.0 / batch_size) * torch.nn.functional.triplet_margin_loss(anchor_var.t(), posource_var.t(),
                                                                                      neg_var.t(),margin=margins[pyramid]).mul(weights[pyramid])
...
class KeypointLoss(object):
    ...
    def __call__(self, data):
        ...
        loc_mat_abs = torch.abs(
            data['target_coord_warped'].view(B, 2, -1).unsqueeze(3) - data['target_coord'].view(B, 2, -1).unsqueeze(2))
        ...
        loc_min_mat = torch.repeat_interleave(l2_dist_loc_min.unsqueeze(dim=-1), repeats=l2_dist_loc_mat.shape[-1],
                                              dim=-1)
        ...
        # desc Head Loss, per-pixel level triplet loss from https://arxiv.org/pdf/1902.11046.pdf.
        desc_loss, _ = build_descriptor_loss(data['source_desc'], data['target_desc_warped'],
                                             data['target_coord_warped'].detach(), top_kk=data['border_mask'],
                                             relax_field=8)
        ...
        repeatable_constrain = ((target_score_associated[dist_norm_valid_mask]
                                 + data['source_score'][dist_norm_valid_mask]) * (loc_err - loc_err.mean())).mean()

        # consistent_constrain in score_loss
        consistent_constrain = torch.nn.functional.mse_loss(
            data['target_score_warped'][data['border_mask'].unsqueeze(1)],
            data['source_score'][data['border_mask'].unsqueeze(1)]).mean() * 2

        score_loss = repeatable_constrain + consistent_constrain
